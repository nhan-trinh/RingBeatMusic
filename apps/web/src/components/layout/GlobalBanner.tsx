...
export const GlobalBanner = () => {
  ...
  useEffect(() => {
    ...
    socketService.on('system_settings_updated', handleUpdate);
    return () => {
      socketService.off('system_settings_updated', handleUpdate);
    };
  }, [fetchSettings]);
  ...
  return (
    <AnimatePresence>
      {(!isDismissed && settings?.global_banner_enabled && settings.global_banner_text) && (
        <motion.div 
          initial={{ scaleX: 0, opacity: 0 }}
          animate={{ scaleX: 1, opacity: 1 }}
          exit={{ scaleX: 0, opacity: 0 }}
          transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
          className="relative isolate overflow-hidden bg-[#1db954] px-6 py-3 sm:px-3.5 sm:before:flex-1 z-50 shadow-[0_0_40px_rgba(29,185,84,0.4)] origin-center"
        >
          {/* Lightning Flash Effect */}
          <motion.div 
            animate={{ 
              opacity: [0, 0.4, 0, 0.2, 0, 0.8, 0],
              backgroundColor: ["transparent", "#fff", "transparent", "#fff", "transparent", "#fff", "transparent"]
            }}
            transition={{ 
              duration: 2, 
              repeat: Infinity, 
            ...
            <div className="flex items-center gap-4 flex-1">
              ...
              <div className="flex items-center gap-3">
                <motion.div
                  ...
                  transition={{ duration: 0.2, repeat: 5, repeatDelay: 1 }}
                >
