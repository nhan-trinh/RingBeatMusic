...
export const ArtistContextMenu = ({
  ...
  const { toggleFollow, isFollowing } = useLibraryStore();
    ...
    >
      ...
      <div className="relative z-10">
        <div className="px-4 py-3 border-b border-white/10 bg-white/[0.02]">
          <p className="text-white font-black uppercase tracking-tighter truncate text-[11px]">{artist.name}</p>
          <p className="text-white/30 text-[8px] font-black uppercase tracking-widest mt-0.5">Creator_Node</p>
        </div>

        <div className="py-2">
          <MenuItem 
            icon={<Heart size={14} className={isFollowingArtist ? "fill-current text-[#1db954]" : ""} />} 
          ...
          <MenuItem 
            ...
            index="RPT"
            onClick={() => {
              openReportModal(artist.id, 'USER', artist.name);
              onClose();
            }} 
          />
        </div>
...
export const useArtistContextMenu = () => {
  const [menu, setMenu] = useState<{ artist: Artist; position: { x: number; y: number } } | null>(null);

  const openArtistMenu = (e: React.MouseEvent, artist: Artist) => {
    e.preventDefault();
    e.stopPropagation();
    setMenu({ artist, position: { x: e.clientX, y: e.clientY } });
  };

  const closeArtistMenu = () => setMenu(null);

  return { menu, openArtistMenu, closeArtistMenu };
};
